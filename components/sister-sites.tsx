    ...
    <section id="companies" className="py-20 md:py-28 bg-muted/30 scroll-mt-28 md:scroll-mt-36">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-4xl mx-auto mb-16">
          <span className="text-sm font-semibold text-accent uppercase tracking-wider">{t('sisterSites.eyebrow')}</span>
          <h2 className="text-3xl md:text-5xl font-bold mt-3 mb-4 text-pretty">
            {t('sisterSites.title')}
          </h2>
          <p className="text-lg text-muted-foreground">
            {t('sisterSites.subtitle')}
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {sisterSiteConfig.map((site) => {
            const siteMessages = messages[locale].sisterSites[site.key]
            const name = t(`sisterSites.${site.key}.name`)
                ...
                <div className="relative p-8 md:p-10 text-white min-h-[320px] flex flex-col">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      <p className="text-white/80 text-sm md:text-base max-w-sm">
                        {t(`sisterSites.${site.key}.description`)}
                      </p>
                    </div>
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap gap-2 mb-6">
                      {siteMessages.features.map((feature) => (
                        <span
                          key={feature}
                  ...
                  <div className="flex items-center gap-2 text-sm font-semibold group-hover:gap-3 transition-all">
                    <span>{t('sisterSites.visit')} {name}</span>
